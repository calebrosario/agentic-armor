...
export const TOOL_DEFINITIONS = [
  {
    ...
    execute: async (params: Record<string, any>) => {
      try {
        ...
        logger.info('Task sandbox created', { taskId: task.id, name: task.name });
        ...
        logger.error('Failed to create task sandbox', { error: error.message });
  ...
  },
  {
    ...
    execute: async (params: Record<string, any>) => {
      try {
        const task = await taskLifecycle.startTask(params.taskId, params.agentId);

        logger.info('Agent attached to task', { taskId: params.taskId, agentId: params.agentId });
        ...
        logger.error('Failed to attach agent', { error: error.message });
        throw new Error(`Failed to attach agent: ${error.message}`);
      }
    },
  },
  {
    ...
    execute: async (params: Record<string, any>) => {
      try {
        ...
        logger.info('Agent detached from task', { taskId: params.taskId, agentId: params.agentId });
        ...
        logger.error('Failed to detach agent', { error: error.message });
        throw new Error(`Failed to detach agent: ${error.message}`);
      }
    },
  },
  {
  ...
  },
  {
    ...
    execute: async (params: Record<string, any>) => {
      try {
        ...
        logger.error('Failed to get task status', { error: error.message });
  ...
  },
  {
    ...
    execute: async (params: Record<string, any>) => {
      try {
        ...
        logger.error('Failed to stop task', { error: error.message });

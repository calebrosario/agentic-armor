...
describe('Concurrency System', () => {
  ...
  describe('OptimisticLock', () => {
    it('should acquire exclusive lock successfully', async () => {
      const lockInfo = await lock.acquireLock('resource1', 'user1', 'exclusive');
    ...
    it('should handle collaborative mode', async () => {
      ...
      const lockInfo = await lock.acquireLock('resource1', 'user2', 'collaborative');
    ...
    it('should prevent unauthorized lock release', async () => {
      ...
      await expect(
        lock.releaseLock('resource1', 'user2')
      ).rejects.toThrow('Cannot release lock owned by \'user1\'');
    });
  ...
  describe('LockManager', () => {
    it('should acquire lock with retry', async () => {
      const lockInfo = await lockManager.acquireLockWithRetry('resource1', 'user1');
      expect(lockInfo.owner).toBe('user1');
    });
  ...
  describe('Integration Test', () => {
    it('should handle concurrent access patterns', async () => {
      ...
      const promises = [
        ...
        lockManager.withLock('shared-resource', 'user2', async () => {
          await new Promise(resolve => setTimeout(resolve, 10));
          return 'user2-result';
        }, 'collaborative'),

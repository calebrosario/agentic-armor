...
describe('TaskLifecycleHooks', () => {
  ...
  describe('Hook Registration', () => {
    test('should register beforeTaskStart hook', () => {
      ...
      expect(taskLifecycleHooks.getHooksByType('beforeTaskStart').length).toBe(1);
    });
    ...
    test('should register afterTaskStart hook', () => {
      ...
      expect(hookId).toMatch(/^hook_\d+$/);
      expect(taskLifecycleHooks.getHooksByType('afterTaskStart').length).toBe(1);
    });
    ...
    test('should register beforeTaskComplete hook', () => {
      ...
      expect(hookId).toMatch(/^hook_\d+$/);
      expect(taskLifecycleHooks.getHooksByType('beforeTaskComplete').length).toBe(1);
    });
    ...
    test('should register afterTaskComplete hook', () => {
      ...
      expect(hookId).toMatch(/^hook_\d+$/);
      expect(taskLifecycleHooks.getHooksByType('afterTaskComplete').length).toBe(1);
    });
    ...
    test('should register beforeTaskFail hook', () => {
      ...
      expect(hookId).toMatch(/^hook_\d+$/);
      expect(taskLifecycleHooks.getHooksByType('beforeTaskFail').length).toBe(1);
    });
  ...
  describe('Priority Ordering', () => {
    test('should execute hooks in priority order (lowest first)', async () => {
      ...
      taskLifecycleHooks.registerBeforeTaskStart(async () => { executionOrder.push(10); }, 10);
      taskLifecycleHooks.registerBeforeTaskStart(async () => { executionOrder.push(1); }, 1);
      taskLifecycleHooks.registerBeforeTaskStart(async () => { executionOrder.push(5); }, 5);
  ...
  describe('Hook Management', () => {
    test('should unregister hook by ID', () => {
      ...
      expect(taskLifecycleHooks.getHooksByType('beforeTaskStart').length).toBe(0);
    });

    test('should get all registered hooks', () => {
      taskLifecycleHooks.registerBeforeTaskStart(jest.fn().mockResolvedValue(undefined));
      taskLifecycleHooks.registerAfterTaskStart(jest.fn().mockResolvedValue(undefined));
      taskLifecycleHooks.registerBeforeTaskComplete(jest.fn().mockResolvedValue(undefined));
      const allHooks = taskLifecycleHooks.getAllHooks();
      expect(allHooks.length).toBe(3);
      expect(allHooks.map(h => h.type)).toEqual(['beforeTaskStart', 'afterTaskStart', 'beforeTaskComplete']);
    });
  });
});
